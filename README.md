...
npm test              # 29 步全套（含 lint 三条 + 最后一次真打包 + 十一组牙齿测试）
...
这一节只验了「**没扫到**东西时会响」，没验「**看见违规**时会红」——
那是第 23 步 `tools/test-lint-rules-teeth.mjs` 的活：沙盒里给三条 lint
各造一处真违规（裸 `fetch`、白名单第 7 处、导入没导出的名字、指向不存在的文件、
引用没定义的样式键），要求它们必须红；**每条都配一个"正确写法要绿"的对照组** ——
少了对照组，"它红了"说明不了任何事，一个无脑报错的检查也能混过去。
它自己的牙齿也验过（变异测试）：把三条规则分别改成不生效，这一步立刻变红。

第 22 / 23 步管的是那三条 lint，本地**第 1 步** `check-persona-coverage` 那张报表
本身还空着 —— 第 24 步 `tools/test-persona-coverage-teeth.mjs` 补的就是它：
沙盒里改一个角色，让报表里的每一项都被要求过一次

| 造出来的缺口 | 报表要说的话 |
|---|---|
| 删掉 `tagline` | 这个角色 ✗ 有缺口 |
| `bgId` 写了背景表里没有的值 | 打出那个不存在的 bgId |
| `idlePose` 指向 `tag:'idle'` 之外的姿势 | 打出那个姿势 id |
| 两个角色撞同一个签名姿势 | 「签名姿势重复」 |
| 删掉 `realistic-noa` 整块 | 「✗ 脚本有问题，别信上面的结果」，且**退出码非 0** |

最后一条是补上一次的账：那句警告以前只是打印，退出码照旧 —— 现在它必须能把
这一步拉红。对照组同样是「原样必须绿」。变异测试（把这五条规则分别改成不生效）
五个变异体全部被杀死。

第 25 / 26 步是同一族的**两个**：`tools/inspect-character.mjs`（本地第 11 步）和
`tools/test-realistic-pipeline.mjs`（本地第 12 步）。它们都是「遍历 `assets/models2`
→ 收集 → 逐项打分 → 打印结论」，是**最容易静默假绿的形态**：目录空了、名字不对，
收集环节就产出 0 项，然后照样打印「全部通过。」。

`inspect-character` 那边的洞是**实测出来的**，不是推出来的 —— 它算完了 `fails`、
打印了「N 项未通过。」，结尾却完全没有 `process.exit`。于是：

| 场景 | 原来的结论 | 真相 |
|---|---|---|
| `assets/models2` 是空目录 | 「全部通过。」退出 0 | 一件东西都没判过 |
| 指定了不存在的角色名 | 打 ✗，但仍打印「全部通过。」退出 0 | 明明体检出不合格 |
| 体检出 8 项不达标 | 「8 项未通过。」退出 0 | 管线当作通过了 |

第 25 步 `tools/test-inspect-character-teeth.mjs` 补的就是这三条 + 「-health 之外的
老 mdl-* 模型不许被当成合格」。这里有个**方法论上的取舍**：把模型弄坏的方式是
**改模型本身**（把 GLB 顶点 Y 压到 0.4 倍，实测真的改到了 12919 个顶点），而不是
把合格门槛改严 —— 后者验的是「门槛还是严的」，前者验的是「脚本真的在读几何」。

第 26 步 `tools/test-realistic-pipeline-teeth.mjs` 是同一族的第二个。它**原本就有**
退出码（缺 GLB 会计进 `totalFails`），缺的只是「一个都没验到」那道 0 条闸门。
拿 HEAD 上的旧版跑过反向验证，正好把两者的差别量出来：

- 目录空 → 旧版**退出 0 并打印「全部通过。」**（这就是那条假绿）；
- 目录不在 → 旧版虽然退出 1，但甩的是 scandir 的 `ENOENT` 堆栈，一句人话没有；
- GLB 截断、缺 GLB 这两种场景旧版**本来就会红** —— 所以这次补的是那两道闸，
  不是把整个脚本重做了一遍。

> ⚠️ 一处容易被自己坑到的地方：判据写成宽松的 `/全部通过/` 会**假红** ——
> 因为脚本在解释「为什么不算通过」的报错文案里自己也会提到这四个字。
> 收紧到 `/^ {2}全部通过。$/m`（只认**结论行**）才是对的。
> 这和 `lint-ci-refs` 把「注释里提到的名字」当成真步骤是同一类错误。

> ⚠️ 另一个坑：这个沙盒必须把 `src/` 一起搬过去（71 个文件）。被检脚本直接
> `import` 项目源码，只复制 `tools/` 下那一个文件会崩在 `ERR_MODULE_NOT_FOUND`
> —— 看上去像「体检没通过」，其实是环境没搭起来，两者必须分得开。
> 而 `src/` 每搬一次都要重建，**每个场景都重建一轮会跑到 20 分钟以上**
> （同一批场景手工跑只要 0.3 秒），所以沙盒只建一次、场景间只换 `models2`。

四个变异体（把两道闸门和退出码分别改成不生效）**全部被杀死**；其中「缺 GLB 不计入
totalFails」是靠单独加的混合名单场景（`noa` + `nosuchhero`）才杀死的 ——
只用「整个名单都不存在」那种场景，会被 0 条闸门**抢先拦下**，看不出这条闸有没有坏。

第 27 步 `tools/test-step-timeout-teeth.mjs` 盯的不是某个脚本，而是**这套东西自己**：

2026-09-26 那次，本地全套跑到第 19 步**挂住**，一挂 4 小时 17 分，日志停在
「PASS lint-ci-refs」之后再无输出。进程 CPU 时间几乎为 0、也没有任何子进程 ——
卡在 Node 自己的 `fs.rmSync(recursive)` 上（沙盒删得只剩一个 yml 就不动了）。

**这一步之前，「挂住」既不是成功也不是失败，而是永远没有结论。** 在 CI 上等价于
烧掉 6 小时的 job 超时，而且红得毫无信息量。现在每一步都有硬上限（10 分钟），
超时按失败算，并额外打印一句「超时 ≠ 通过，也 ≠ 这次不算」。

> ⚠️ 两件事要说清楚，免得把它当成修好了根因：
>   ① **根因没锁定**。换个新进程删同一个残留只要 64ms；造同样形状的目录删也只要
>      138ms；4 个变量组合（文件数 / 重写次数）全部飞快 —— 最小复现不成立。
>      能确定的只有「一步挂住 → 整套没有结论」，补的就是这个洞。
>   ② **超时必须杀进程树，不能只杀直接子进程**。Windows 上 `child.kill()` 只杀一个，
>      孙进程会留下来继续占着文件句柄 —— 而最可疑的那次恰好就是被句柄卡住的删除。
>      所以用的是 `taskkill /PID <pid> /T /F`，牙齿里也有一条「孙进程心跳要停」盯着。

四个变异体（超时伪装成 exit 0 / 只标记不真杀 / 退出码被抹平 / 定时器整个拿掉）
**全部被杀死**；其中第四个会让牙齿脚本自己也挂住，所以变异器有一层独立实现的外层
上限 —— 不能复用被改坏的那个执行器。

### 第 28 步：谁来提醒「第 29 步还没配牙齿」

前面 19~27 步每一条牙齿，都是**补完一条闸之后、由我想起来**才去配的。
补完第 27 步之后这个问题就摆在那了：

> 将来再加第 28、29 步时，谁来提醒它「你还没配牙齿」？

答案是「我」—— 而「下次记得」不算防线（这条纪律在这个仓库里已经反复应验过）。
所以第 28 步把那个提醒变成机器的事：**`tools/runtests.mjs` 里每一步都必须登记
`teeth` / `guardedBy` / `why` 三选一，漏一个整套测试拒绝开跑。**

```
$ node tools/runtests.mjs --check-registry

【登记核对】共 29 步：18 步有常驻牙齿，11 步只靠理由
  ⚠️ 其中 19 步自认「未验证」（只是我的判断，没有证据）：withTimeout / ...
  ✓ 登记齐全 —— 开跑
```

加第 29 步的人会当场撞上这个，不需要谁记得：

```
  ✗ brand-new-step：没有登记 —— 加这一步的人必须写明它的牙齿在哪，或者为什么不需要（不写就跑不起来）
  ✗ 登记有 1 项问题 —— 整套测试**拒绝开跑**
```
...
| 字段 | 填什么 | 机器会核对什么 |
| --- | --- | --- |
| `teeth` | 它的牙齿脚本路径 | 脚本真的存在；**并且**清单里真有 `<name>-teeth` 这一步（牙齿不跑等于没有） |
| `guardedBy` | 已有谁在盯它（可多个） | 指向的步骤名都在清单里 |
| `why` | 为什么不需要牙齿 | 少于 10 个字要红；「不需要 / 待补 / TODO」这类占位要红 |

**为什么不是"自动扫代码判断谁有牙齿"**：试过，判不准。九条牙齿里有四条的名字跟
被保护的步骤对不上（`lipsync-teeth` 盯的是 `pipeline`；`zero-scan-teeth` 盯的是
四条 lint；`persona-coverage-teeth` 盯的是 `check-persona-coverage`）。
"看到某个文件里提到了另一个文件名"根本推不出"这条在保护那条"。所以改成**显式
登记**：谁保护谁由登记的人写清楚，机器只负责核对没漏。

⚠️ 改写法这件事连带弄坏了**两处**，都值得单独记一笔：

1. 第 18 步 `lint-ci-refs.mjs` 里那条 `\['([^']+)',\s*\[([^\]]*)\]\]` 一条都扫不到了 ——
   表现为「本地套装有 0 步」，B/C 两项**整层静默跳过**，只剩 A 项在跑。现在两种写法都认。
   反向验证过：退回旧正则，这一步立刻 28 条 ✗。
2. 第 19 步 `test-lint-ci-refs-teeth.mjs` 的 C3 场景要在沙盒副本上给某一步加一个参数，
   它匹配的是 `\['withTimeout', \['tools\/test-with-timeout\.mjs'\]\]` —— 同样匹配不到了。
   **这一处是本地跑全套时第 19 步自己变红才发现的**，我事先只想到第 1 处。
   现在只匹配 `'名', [...]` 这一段，两种写法都认。

> 教训：改「清单长什么样」之前，要把**所有按旧形状匹配它的地方**扫一遍 ——
> 全仓库扫 `runtests` 关键字，两处都在这次扫描里。而真正的保险不是我扫得全，
> 是**第 19 步会红**：它挂在沙盒副本上按正则改文件，改不到就抛错，于是当场响。

**牙齿测试**（42 项断言，分两半）：

- **甲·纯函数层**：拿合成清单喂 `verifyRegistry`，确认 11 类坏登记都会红
  （漏登记 / teeth 指向不存在 / 清单里没有 `-teeth` / 占位理由 / 理由太短 /
  两个字段都填 / 孤儿登记 / guardedBy 指向不存在 / **空清单** / **重名**）。
  后两条是写牙齿时才发现的洞：清单空了、或者两步重名，原来的实现会返回「0 问题」——
  又是一次「扫到 0 条就静默全绿」。
- **乙·真跑层**：把 `runtests.mjs` 等三个文件复制到沙盒，**真的 spawn 一次**
  `node runtests.mjs --check-registry`，要求退出码非 0 且报错点名那一步。
  只做甲不够 —— 甲验的是函数，验不到「整套拒绝开跑」这个行为本身。
  每个场景配对照组：原样必须 exit 0 且打印「登记齐全」。

11 个变异体（九条闸分别拿掉 + 「无论如何都拒绝」+「永远放行」）**全部被杀死**。
其中「永远放行」那一个让 31 项断言变红，「无论如何都拒绝」那一个连对照组一起红 ——
后者证明对照组不是摆设。

> 两个踩坑，都值得记：
>   ① **沙盒不完整会造出假红**。第一版沙盒只复制了三个文件，于是核对把
>      「teeth 指向的脚本不存在」报成问题，**对照组自己就红了** —— 而它红的
>      原因跟「登记」毫无关系。这种假红最害人，它会让人以为闸在工作。
>      现在沙盒会把登记里引用的每个牙齿脚本一起复制过去。
>   ② **变异的插入点必须在 `verify()` 之前**。第一版插在 `if (CHECK_ONLY)` 前，
>      而核对在那之前已经跑完了 —— 结果五个场景的 exit 都是非 0（被别的真问题带红的），
>      却全都点不名，「通过」得毫无意义。**只验「它红了」而不验「它为什么红」，
>      会连插错地方都发现不了。**

**这份欠账单是故意留在屏幕上的**：每次跑全套都会打印那几步「只靠理由、没有常驻
牙齿」的清单。想让它变短，唯一的办法是去给那些步骤配牙齿，而不是把这一栏删掉。

> ⚠️ 2026-09-26 补记：这道闸**曾经只在本地生效**。CI 的 test job 是自己在 bash 里
>   逐步跑的，从没调用过 `runtests.mjs` —— 所以「漏登记就拒绝开跑」在 CI 上
>   一遍都没执行过。已补（见第 29 节末尾那段），并由 `lint-ci-refs` 的【D】项
>   盯着那行还在不在。

剩下的 11 步**全部是牙齿脚本自己** —— 牙齿的牙齿到此为止，再往上就是无限递归，
这一层目前没有任何常驻检查，靠的是「它们当初生效时做过变异测试」这个当时的证据。

### 第 29 步：把那份欠账单上的「未验证」一条条证掉

欠账单摆上屏幕之后，那 9 条 `why` 里我写的是「它断言的是真实行为，断言写坏会红」——
旁边老实标了「未验证」。**未验证的断言不算数**，所以第 29 步用变异去证它们。
第一批四个（`nativeSpeech` / `espeak` / `tts` / `songs`）的结果是：

| 步骤 | 造的事故 | 结果 |
| --- | --- | --- |
| `tts` | 清空 `EMOTIONS` | 13 项 ✗ —— ✅ 我的判断是对的，它真有牙齿 |
| `nativeSpeech` | 清空 `NATIVE_EMOTION` | 5 项 ✗ —— ✅ 同上 |
| `espeak` | 粤语嗓音码改回 `yue` | 点名 `zh-hk` 的 ✗ —— ✅ 这就是它存在的理由 |
| `songs` | 清空 `SONG_LIST` | **「曲库 0 首」，WAV 那批实质判据整批被跳过** —— ❌ 洞 |

**那个洞长这样**：`songs.test.mjs` 遍历 `SONG_LIST` 验每一首的 WAV 头 / 峰值 /
有声占比。曲库空了，这个循环一次都不跑，`fail` 保持 0，脚本一路走到模糊匹配，
最后**靠 `matchSong` 返回 undefined 抛 `TypeError` 才退出非 0** —— 而那跟
"歌有没有声音"毫无关系。

已补两条：① 显式 0 条闸门 `ck(SONG_LIST.length > 0, '曲库非空')`；
② 随机兜底从 `matchSong(...).title` 改成 `?.title`，**崩溃改成一条 ✗**。

> ⚠️ **「靠崩溃才红」是 fragile 的，不能算防线。** 哪天有人给 `matchSong` 加个
> 空表兜底（非常常见的"健壮性"改法），崩溃就没了，静默全绿立刻回来。
> 所以第 29 步额外要求：songs 的失败必须是一条 ✗，输出里**不许出现 `TypeError`**。

**牙齿测试**（对照组 ×4 + 变异 ×4 + 变异有效性 ×4 + 收工核验 ×4）：

- 对照组：四个步骤原样都要 exit 0 且没有失败行。
- 变异：四个步骤看见各自的事故都要 exit ≠ 0，且报错要点名。
- ⓕ 变异必须真的改到了东西（否则"绿"等于没验过）。
- ⓖ **收工前四个源文件必须与原文逐字节一致**（sha256）。这一步会临时改真实
  源文件 —— 第 21 步用过的做法，三重保险：内存留原文 / 每场景 `try/finally`
  还原 / 收工比对。**清理不干净比测试失败更糟**：脏的源文件会让后面每一步的
  结论都不可信。

**变异测试 7 个，6 杀 1 不杀**：

- 杀掉的里最关键的是 `T1 拿掉 songs 的 0 条闸门（回到修复前）` —— 它证明这条
  牙齿**真的在守那个洞**，而不只是"会跑"。另有 `T2 让随机兜底变回崩溃`、
  `M4 拿掉每个场景里的还原（让源文件脏着）`。
- 杀不掉的是 `M5 收工核验改成不报`：还原**总是成功**时那条核验本来就通过，
  它不可能被这个变异触发；真正盯住它的是 M4。**这一个我如实标成"未杀死"**。

> 又踩一次同一个判据坑：第一版用裸 `/✗/.test(out)` 判「有没有失败」，结果命中了
> 说明文字里我自己写的那个 ✗（`「曲库为空时这里也必须是一条 ✗，不是崩溃」`
> 被打印在一条 **✓** 行上），对照组凭空变红。现在只认**行首两空格 + ✗**
> （`/^ {2}✗/m`）。跟第 25 步「`/全部通过/` 命中了正在解释为什么不算通过的
> 报错文案本身」是同一类：**判据宽到能命中说明文字**。

欠账单因此短了 4 条。

#### 第二批（同日下午）：欠账单上剩下的五个「非牙齿」步骤，9 个常驻变异（连第一批共 13 条，全杀）

> 数字订正过一次：原先写「12 个变异、11 杀 1 存活」，那是把排查阶段的**临时探针**
> 一起算进去的 —— 探针没登记成常驻场景，事后对不上代码也没法复现。
> 登记在案的是**第一批 4 条 + 第二批 9 条 = 13 条，实测 13 条全部被杀死**

第一批证完之后，欠账单上还剩五个**不是牙齿脚本**的步骤。同样的方法再来一轮：

| 步骤 | 造的事故 | 结果 |
| --- | --- | --- |
| `withTimeout` | 超时上限乘 3（450ms） | **原本全绿** —— ❌ 洞 |
| `withTimeout` | 不再包 `Promise.resolve`（thenable 同步抛） | 红，点名 ✅ |
| `auth-timeout` | 归一出来的 `kind` 写成 `network` | 红 ✅ |
| `auth-timeout` | `dbCall` 超时后不 `abort` | 红，点名 ✅ |
| `net-timeout` | 退化成「只发 signal、不做 race」 | 红，点名 ✅ |
| `net-timeout` | 一律按秒取整（80ms → 「等了 0 秒」） | 红，点名 ✅ |
| `rig-semantics` | 拿掉 `out` 对 `fwd` 的正交化 | **原本全绿** —— ❌ 盲区 |
| `rig-semantics` | 允许连着播同一个姿势 | **原本全绿** —— ❌ 碰运气的断言 |
| `bundle` | 引用一个不存在的模块（CI #25 的形状） | web 5.2s / ios 3.0s 双双失败 ✅ |

三个洞，逐个说清楚：

**① `withTimeout` 的耗时窗口有 6 倍宽。** 原断言写的是
`dt >= 140 && dt < 900`，而请求的上限只有 150ms —— 实测实际耗时 155~166ms，
也就是说**上限在 6 倍范围内自由漂移都不会红**。而调等待上限恰恰是弱网调优
最爱动的地方。

补的断言**不看墙钟**，直接盯「排进计时器的那个毫秒数」：把 `globalThis.setTimeout`
临时换成记录器，要求排进去的必须就是传进去的 150ms。乘倍数、加常数、换单位
都躲不掉，而且跟机器快慢无关，**永远不会飘**。

> 为什么不去收紧 `[140,900)`：那样只能按倍数往下压，压到 2 倍就开始跟机器
> 快慢赛跑。而一条**会随机变红**的断言比没有断言更糟 —— 它会教会所有人
> 「红了就重跑一次」。

**② 那两棵合成骨架够不到正交化失效的状态。** `semAxes.js` 顶部记着一次
「Quaternius T-pose 骨架上前臂残差 102°」的事故，可实测把
`out.addScaledVector(fwd, -out.dot(fwd))` 整行删掉，69 项照旧全绿。

原因是能算出来的：`out_raw · fwd = away_z − (away·d)(F·d)`，而 `DEFS` 里
**每根骨的 z 都是 0**，右边恒等于 0 —— 两棵树上手臂从来不往身前伸，
正交化是空操作。补了一棵**手臂带 z 分量**的骨架（手往身前伸），
让 `away` 和 `F` 在「垂直于骨段的平面」里不再正交。

**③ 「没有连着播同一个姿势」是一条碰运气的断言。** 把去重
（`all.filter(p => p.id !== lastId)`）拿掉，它照样绿 —— 18 个姿势随机抽七八次，
撞上同一个的概率本来就不高。它绿了不代表去重还在，真红了也会被当成随机波动。
已把那一段的 `Math.random` 钉成常数：**有去重时在两个中间位之间来回摆，
去重一拿掉就永远停在同一根** —— 概率问题由此变成确定性问题。

**唯一存活的那一条不用补**（⚠️ 它是排查时的**临时探针，没有登记成常驻场景**，
所以不在上面的表里，也拿不到证据复现）：拿掉 `guard.catch(() => {})` 仍然是绿的，
而且**绿是对的** —— race 一建立 guard 就有人接了，那条 catch 只在
「race 建立之前就同步抛错」时才用得上，而 `Promise.resolve(source)` 不会抛。
它是纵深防御，不是当前路径上的判据；硬要它红反而是造假。

**这一轮的代价**：第 29 步从约 15 秒变成约 41 秒（多出来的主要是 bundle 那一条：
对照组 3.1 秒 + 变异 5.2 秒）。全套从 2m02s 变成约 2m45s。

**顺带修掉的一个假红**：`bundle` 那条不能不带 `--keep` —— 不带时
`verify-bundle` 收尾的 `fs.rmSync(dist-localcheck-web)` 在这台机器上**挂住不返回**，
对照组跑到 180 秒硬上限被杀。打包其实 3.1 秒就成功了，卡的是清理。

**欠账单上「非牙齿步骤」已清零**：29 步里 18 步有常驻牙齿，剩下 11 步
**全部是牙齿脚本自己**（牙齿的牙齿到此为止）。

**CI #62（`b57c08d`）上已取到证据**：三 job 全 success，test job 里
29 条 `PASS`、0 条 `FAIL`；新补那条不看墙钟的断言在 CI 上也是绿的
（`✓ 排进计时器的就是传进去的那 150ms（实际 150）`）；第 29 步自己那一行的
收尾是「九个源文件已逐字节还原」（sha256 逐条对上）。

#### 同一轮查出来的另一个缺口：那道闸在 CI 上根本没执行

在 CI #61 的日志里翻证据时发现：workflow 的 test job 是**自己在 bash 里逐步跑**
的（一串 `run "名字" node ...`），**从来没有调用过 `tools/runtests.mjs`** ——
于是第 28 步那句「漏登记就整套拒绝开跑」在 CI 上一遍都没执行过。

后果是：本地忘了给新步骤写牙齿会被拦下，CI 上一个没写牙齿的步骤照样能加进来
并且全绿（`lint-ci-refs` 只对照「名字 + 命令行」，不看牙齿在哪）。

修法是两件事一起做：① workflow 里加一行
`node tools/runtests.mjs . --check-registry`（只核对登记、不跑任何一步，约 1 秒）；
② `lint-ci-refs` 加【D】项盯住那一行还在不在 —— **不然它跟别的「下次记得」
一样，会被无声删掉**。第 19 步的牙齿同步加了场景 `D3`（把那行删掉必须红），
9 个场景全过。

> 这一项能盯到什么程度，说清楚：它只认「命令行里出现了这段」。如果有人把它
> 包进 `echo "..."`，这一项照样绿 —— 防的是**整行被删/被改坏**，
> 不防「写了却不执行」。

**CI #62 上已翻到它在真跑的证据**：test job 日志里

```
14:51:58.1942086Z node tools/runtests.mjs . --check-registry   ← ##[group] 内，是脚本内容
14:51:58.2070479Z ##[endgroup]
14:51:58.2496782Z 【登记核对】共 29 步：18 步有常驻牙齿，11 步只靠理由
14:51:58.2500530Z   ✓ 登记齐全 —— 开跑
```

`【登记核对】` 出现在 `##[endgroup]` **之后**，是命令真的被打出来的输出，不是回显。
而且整个 job 日志里 `【登记核对】` **只出现 1 次** —— 若它是别的步骤（比如
`step-registry-teeth` 的沙盒）顺带打出来的，时间不会落在这一步刚开跑的 14:51:58。
同一次运行里还有 `【D】CI 上必须真的跑一次登记核对` +
`✅ D3 ★ CI 上跑登记核对的那一行被删掉时必须响`。
...
| job | 干什么 | 失败后果 |
|---|---|---|
| `test` | 与 build **并行**跑全套测试（29 步） | **不挡发布**，只把这个 job 标红 |
